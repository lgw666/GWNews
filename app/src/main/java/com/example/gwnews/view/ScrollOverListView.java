...
public class ScrollOverListView extends ListView {
    ...
    public void setTopPosition(int index){
        if(getAdapter() == null)
            throw new NullPointerException("必须在设置顶部位置之前设置适配器!");
        if(index < 0)
            throw new IllegalArgumentException("顶部位置必须大于0！");
    ...
    public void setBottomPosition(int index){
        if(getAdapter() == null)
            throw new NullPointerException("必须在设置Button的位置之前设置适配器!");
        if(index < 0)
            throw new IllegalArgumentException("Button的位置必须大于0！");
